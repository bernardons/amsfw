...
public abstract class AbstractIntegrationTest {
	...
	private static String getDbURL() {
		StringBuffer dbURL =
				new StringBuffer("jdbc:h2:mem:test;DB_CLOSE_DELAY=-1;DB_CLOSE_ON_EXIT=FALSE;MODE=PostgreSQL;");
	...
	private static void configureDBUnit() throws Exception {
		...
		try {
			...
			connection.getConfig().setProperty("http://www.dbunit.org/features/qualifiedTableNames", true);
			databaseSequenceFilter = new DatabaseSequenceFilter(connection);
	...
	/**
	 * Cria um {@link IDataSet} do tipo {@link FlatXmlDataSet} com o nome do
	 * arquivo informado.
	 * 
	 * @param fileName
	 *            Arquivo xml
	...
	protected static IDataSet getFlatXmlDataSet(String fileName) throws Exception {
		...
		ReplacementDataSet dataSet = new ReplacementDataSet(new FilteredDataSet(databaseSequenceFilter, xmlDataSet));
	...
	protected IDataSet getDataSets() throws Exception {
		...
		Collection<String> xlsBuilders =
				TestBuilderUtil.getAllBuilders(null, getTestBuilderList(), new ArrayList());
