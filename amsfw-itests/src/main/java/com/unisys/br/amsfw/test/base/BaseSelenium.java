...
public abstract class BaseSelenium {
	...
	protected WebElement encontrarCheckPorCss(String elemento) {
		return driver.findElement(By.cssSelector(elemento));
	}

	protected WebElement encontrarSelectPorCss(String elemento) {
		return driver.findElement(By.cssSelector(elemento));
	}
	...
	protected void encontrarAutoCompletePorIdRegexContendo(String elemento, String value) throws InterruptedException {
		driver.findElement(By.cssSelector(elemento)).sendKeys("");
		driver.findElement(By.cssSelector(elemento)).sendKeys(value);
		Thread.sleep(TEMPO_CARREGAMENTO_AUTO_COMPLETE);
		driver.findElement(By.cssSelector(elemento)).sendKeys(Keys.TAB);
		Thread.sleep(AGUARDAR_RESPOSTA_AJAX);
	}
	...
	/**
	 * Verifica se um <i>CheckBox</i> por css esta com o atributo readonly igual
	 * a true.
	 * 
	 * @param campo
	 * @return true caso o <i>input</i> esteja readonly
	 */
	protected boolean testReadOnlyCheckBoxPorCss(String campo) {
		if (getDriver().findElement(By.cssSelector(campo)) != null
				&& encontrarCheckPorCss(campo).getAttribute("readonly") != null
				&& encontrarCheckPorCss(campo).getAttribute("readonly").equals("true")) {
			return true;
		}
		return false;
	}
	...
	/**
	 * Verifica se um <i>Select</i> por css esta com o atributo readonly igual a
	 * true.
	 * 
	 * @param campo
	 * @return true caso o <i>input</i> esteja readonly
	 */
	protected boolean testReadOnlySelectPorCss(String campo) {
		if (getDriver().findElement(By.cssSelector(campo)) != null
				&& encontrarSelectPorCss(campo).getAttribute("readonly") != null
				&& encontrarSelectPorCss(campo).getAttribute("readonly").equals("true")) {
			return true;
		}
		return false;
	}
	...
	/**
	 * Método que pega o option do select por texto parcial.
	 * 
	 * @author OliveiMH
	 * @param cssSelector
	 * @param textoParcial
	 */
	protected void setSelectValuePartialText(String cssSelector, String textoParcial) {
		WebElement select = getDriver().findElement(By.cssSelector(cssSelector));
		List<WebElement> listaDeOpcoes = select.findElements(By.tagName("option"));
