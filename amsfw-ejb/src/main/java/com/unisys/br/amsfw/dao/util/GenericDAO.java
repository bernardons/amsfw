...
public abstract class GenericDAO<T extends Entidade, C extends Serializable> implements GenericDAOLocal<T, C> {
	...
	public static final Integer MAXRESULTS_DOMINIODISCRETOS = 20;
	...
	/**
	 * Em determinados casos quando existe orderBy por exemplo com essa
	 * propriedade setada como true não funciona. order by items must appear in
	...
	public List<? extends DominioDiscreto> recuperarTodosDominiosDiscretos(Class<? extends DominioDiscreto> object) {
		...
		TypedQuery<? extends DominioDiscreto> q =
				this.getEntityManager().createQuery("select c from " + object.getSimpleName() + " c " + adicionaOrderByHql("id"), object);
