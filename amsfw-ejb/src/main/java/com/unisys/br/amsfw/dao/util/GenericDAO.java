...
public abstract class GenericDAO<T extends Entidade, C extends Serializable> implements GenericDAOLocal<T, C> {
	...
	public List<Object[]> buscarDadosTXT(String campos, String tabela, String where) {
		List<Object[]> lista = null;

		Query q = this.getEntityManager().createNativeQuery("select " + campos + " from " + tabela + " " + where);
	...
	public List<T> recuperarTodos(Order... ordenacoes) throws MuitosResultadosException {
		...
		TypedQuery<T> q = this.getEntityManager().createQuery(
				"select c from " + getPersistenceClass().getSimpleName() + " c " + adicionaOrderByHql(ordenacoes), getPersistenceClass());
	...
	protected List<T> recuperarTodosSemLimite() {
		List<T> retorno = this.getEntityManager()
				.createQuery("SELECT c FROM " + getPersistenceClass().getSimpleName() + " c", getPersistenceClass()).getResultList();
		return retorno;
	}
	 ...
	 * {@inheritDoc}
	 */
	public List<T> listarParaTXT(T entidadeFiltro, String orderBy, Object... filtrosExtras) {
	 ...
	 * @param root
	 */
	protected void recuperaQuery(T entidadeFiltro, CriteriaBuilder criteriaBuilder, CriteriaQuery<?> criteriaQuery, Root<T> root,
