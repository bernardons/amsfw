 ...
 * resultado da tela de pesquisa. 
 * Para usar este freature utilizar o componente exporterPDF .
 * 
 * 
 * 
 * @author EgidioRR
...
public abstract class PDFExporter {
	 ...
	 * @return
	 */
	protected abstract String getBordaCabecalho();
	 ...
	 * @return
	 */
	protected abstract String getBordaTabela();
		...
		for (UIColumn col : table.getColumns()) {
			...
			if (col.isExportable()) {
				band.appendChild(criarCampoTextoOuField(posicaoX, columnWidths[i],
						exportarValor(context, col.getFacet("header")), doc, true, getBordaCabecalho(), col.getStyle()));
	...
	private EntidadeRelatorioGenerico exportarConteudo(DataTable table) {
		...
		for (UIColumn col : table.getColumns()) {
			...
			if (col.isExportable()) {
				retorno.popularProximo(exportarValor(context, recuperarConteudoColuna(col.getChildren())));
			}
		}
	...
	private UIComponent recuperarConteudoColuna(List<UIComponent> children) {
	...
	private Node criarCampoTextoOuField(Integer posicaoX, Integer columnWidths, String texto, Document doc,
		...
		if (cabecalho) {
			...
			conteudo.setTextContent("$F{campo" + texto + "}");
		}
	...
	private void criarBorda(Document doc, String borda, Element tipoCampo) {
		...
		Element bordaDown = doc.createElement("bottomPen");

		Element bordaRight = doc.createElement("rightPen");

		box.appendChild(bordaUp);
		box.appendChild(bordaLeft);
		box.appendChild(bordaDown);
		box.appendChild(bordaRight);
		String[] bordaSplit = borda.split(",");

		bordaDown.setAttribute("lineWidth", bordaSplit[0]);
		bordaLeft.setAttribute("lineWidth", bordaSplit[1]);
		bordaUp.setAttribute("lineWidth", bordaSplit[2]);
		bordaRight.setAttribute("lineWidth", bordaSplit[3]);
