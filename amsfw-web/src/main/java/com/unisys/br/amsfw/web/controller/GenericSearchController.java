...
import com.unisys.br.amsfw.domain.ImpressaoTxtInterface;
import com.unisys.br.amsfw.domain.ObjetoImpressaoTxt;
...
public abstract class GenericSearchController<T extends Entidade, C extends Serializable> implements Serializable {
	...
	public void exportarTXT() {
		...
		if (table.getRowCount() == 0) {
			...
			try {
				...
				List<T> dados =
						getListaDadosParaTXT(getDAO().listarParaTXT(objetoFiltro, null, getFiltrosAdicionais()));

				for (T t : dados) {
					t = adicionarPropriedadesEntidade(t);
					if (t instanceof ImpressaoTxtInterface) {
						ObjetoImpressaoTxt objetoImpressaoTxt = ((ImpressaoTxtInterface) t).imprimirEntidade();
						export.append(objetoImpressaoTxt.getLinha());
						count += objetoImpressaoTxt.getContador();
						export.append("\r\n");
					}
				}
				...
				FacesUtils.disponibilizarArquivoTxtParaDownload(baos);
	...
	/**
	 * Metodo que modifica propriedades da entidade e retorna a mesma.
	 * Caso algum caso de uso tenha que manipular esta lista, este método deve ser sobrescrito.
	 * @param t
	 * @return
	 */
