...
public abstract class GenericReportController<T extends Serializable> implements Serializable {
	...
	/**
	 * Chama o metodo que faz a limpeza da arvore de componentes. Deve ser
	 * sobrescrito pelas classes filhas e invocado.
	 * */
	public void limpar() {
		this.setHasRegistrosDataSource(false);
		pdfByteArray = null;
	...
	private StringBuffer exportReportToHtmlBuffer(JasperPrint print) throws JRException {
		...
		exporter.setParameter(JRHtmlExporterParameter.IMAGES_URI, this.getDiretorioImagensReport());
	...
	protected ByteArrayOutputStream exportToXLS() throws Exception {
	...
	/**
	 * Recupera os bytes do pdf gerado.
	 * 
	 * @return
	 */
	public void exportToPdfByteArray() {
		try {
			if (jasperPrint != null) {
				pdfByteArray = exportToPDF().toByteArray();
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	protected void gerarRelatorio(Map parametros, JRBeanCollectionDataSource dataSource) {
		...
		try {
			...
			parametros.put("rodape_left", this.getRodapeLeft());
			parametros.put("REPORT_LOCALE", new Locale("pt", "BR"));
			parametros.put("confidencialidade", FacesUtils.getMessage("confidencial.10"));
			parametros.put(JRParameter.IS_IGNORE_PAGINATION, isIgnorePagination());
			...
			getFormaDeDisponibilizacaoDoRelatorio();
		} catch (Exception e) {
			e.printStackTrace();
	...
	/**
	 * Define se o relatório irá ou não Ignorar a Paginação. Para relatorios PDF
	 * o default será FALSE Para relatorios XLS o defaul será TRUE.
	 * 
	 * @return
	 */
	protected Boolean isIgnorePagination() {
		return Boolean.FALSE;
	}

	/**
	 * Altera a forma que o relatorio será disponibilizado para cliente.
	 */
	protected void getFormaDeDisponibilizacaoDoRelatorio() throws Exception {
		exportToPdfByteArray();

	}

	public String getNomeLogo() {
		return "caixa_logo.gif";
	}
	...
	/**
	 * Metodo responsavel por setar o rodape do lado esquerdo do relatorio.
	 * 
	 * @return
	 */
	protected abstract String getRodapeLeft();

	/**
	 * Metodo responsavel por setar o diretorio aonde serao salvos as imagens
	 * dos relatorios.
	 * 
	 * @return
	 */
	protected abstract String getDiretorioImagensReport();
