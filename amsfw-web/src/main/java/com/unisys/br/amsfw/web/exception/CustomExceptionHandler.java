...
public class CustomExceptionHandler extends ExceptionHandlerWrapper {
		...
		while (i.hasNext()) {
			...
			try {
				...
				if (throwable instanceof AmsfwException) {
					...
					t.getCause().printStackTrace(new PrintWriter(stringWriter, true));
	...
	private void lancarExcecaoTipoAmsfwException(final FacesContext facesContext,
		final NavigationHandler navigationHandler, Throwable throwable) {

		log.log(Level.SEVERE, "Erro tratado.", throwable);

		if (((AmsfwException) throwable).getParametros() != null
				&& ((AmsfwException) throwable).getParametros().length > 0) {
