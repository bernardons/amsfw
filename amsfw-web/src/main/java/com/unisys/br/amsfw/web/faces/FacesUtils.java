...
public class FacesUtils {
	...
	/**
	 * Adiciona uma mensagem de acordo com a id do componente mensagem e os
	 * detalhes.
	 * 
	 * @param msg
	 * @param detail
	 */
	public static void addErrorMessagePorId(String msg, String id, String detail) {
		FacesMessage facesMsg = new FacesMessage(FacesMessage.SEVERITY_ERROR, msg, detail);
	...
	/**
	 * Adiciona uma mensagem de acordo com a id do componente mensagem.
	 * 
	 * @param msg
	 * @param detail
	 */
	public static void addErrorMessagePorId(String msg, String id) {
	...
	/**
	 * Adiciona uma mensagem ao componente passado por parametro.
	 * 
	 * @param msg
	 * @param idComponente
	 */
	public static void addErrorMessagePorComponente(String msg, String idComponente) {
		UIComponent comp = ComponentUtils.findComponent(FacesContext.getCurrentInstance().getViewRoot(), idComponente);
		if (comp != null) {
	...
	/**
	 * Adiciona uma mensagem de sucesso bundle.
	 * 
	 * @param msg
	 */
	public static void addSuccessMessageBundle(String chave) {
		FacesMessage facesMsg = new FacesMessage(FacesMessage.SEVERITY_INFO, FacesUtils.getMessage(chave), "");
		FacesContext.getCurrentInstance().addMessage(null, facesMsg);
	}
	...
	public static void disponibilizarArquivoXlsParaDownload(ByteArrayOutputStream outputStream) throws IOException {
		disponibilizarArquivoParaDownload(outputStream, "arquivo.xls");
	}
	...
	public static void disponibilizarArquivoPdfParaDownload(ByteArrayOutputStream outputStream) throws IOException {
		disponibilizarArquivoParaDownload(outputStream, "arquivo.pdf");
	 ...
	 * @throws IOException
	 */
	public static void disponibilizarArquivoTxtParaDownload(ByteArrayOutputStream outputStream) throws IOException {
		disponibilizarArquivoParaDownload(outputStream, "arquivo.txt");
	 ...
	 * @throws IOException
	 */
	public static void disponibilizarArquivoZipParaDownload(ByteArrayOutputStream outputStream) throws IOException {
		disponibilizarArquivoParaDownload(outputStream, "arquivo.zip");

	}

	public static void disponibilizarArquivoParaDownload(ByteArrayOutputStream outputStream, String nomeArquivo)
		throws IOException {
		...
		addCookieBlockUiNoResponse(response);
		outputStream.flush();
		outputStream.close();
	...
	/**
	 * Método para disponibilizar arquivos de tamnho grande. A entrada recebe um
	 * InputStream que é carregado diretamente no outputStream do Response.
	 * O InputStream tem que ser fechado pelo método que faz a chamada deste.
	 * 
	 * @param inputStream
	 * @param nomeArquivo
	 * @throws IOException
	 */
	public static void disponibilizarArquivoParaDownload(InputStream inputStream, String nomeArquivo)
		throws IOException {
		HttpServletResponse response =
				(HttpServletResponse) FacesContext.getCurrentInstance().getExternalContext().getResponse();
	...
	public static void addCookieBlockUiNoResponse(HttpServletResponse response) {
		addCookieNoResponse("blockUi", "blockUi", response);
	}

	public static void addCookieNoResponse(String cookie, String valor, HttpServletResponse response) {
		Cookie cookieResponse = new Cookie(cookie, valor);
		response.addCookie(cookieResponse);
	}

	/**
	 * Metodo recursivo para limpar a arvore de componentes do JSF. Varre a
	 * arvore de componentes a partir do form passado. Caso seja passado null
	 * pega por default "formPesquisa" caso o form na tela desejada tenha outro
	 * nome, deve ser passado por parametro exemplo: "formCadastro".
	 * */
	public static void limparComponentesForm(String formTela) {
		...
		if (form == null) {
			form =
					FacesContext.getCurrentInstance().getViewRoot()
							.findComponent(formTela == null ? "formCadastro" : formTela);
		}
	...
	private static void limparForm(UIComponent component) {
		...
		if (UIComponent.isCompositeComponent(component)) {
			@SuppressWarnings("rawtypes") Iterator i = component.getFacetsAndChildren();
			while (i.hasNext()) {
	 ...
	 * @param id
	 */
	public static void mensagemCampoObrigatorio(String id) {
		UIComponent comp = ComponentUtils.findComponent(FacesContext.getCurrentInstance().getViewRoot(), id);
		if (comp != null) {
			FacesContext.getCurrentInstance().addMessage(
					comp.getClientId(),
					new FacesMessage(FacesMessage.SEVERITY_ERROR, null, FacesUtils
							.getMessage("default.required.message")));
		}

	}

	/**
	 * Encontra Componente que possui determinado id.
	 * 
	 * @param UIComponent
	 * @param id
	 * @return
	 */
	public static UIComponent findComponentById(UIComponent c, String id) {
